...
static void tcp_server_task(void *pvParameters) {
  ...
  while (1) {
    ...
    xTaskCreatePinnedToCore(do_retransmit, "tcp_tx", 1 * 4096, (void *)sock, 21,
                            NULL, 1);
  }
...
void esp_event_handler(void *event_handler_arg, esp_event_base_t event_base,
...
void wifi_init_sta(char *ssid, char *pass) {
  ...
  ESP_LOGI(TAG, "wifi_init_sta finished. SSID:%s password:%s", ssid, pass);
}
...
esp_err_t wifi_init() {
  ...
  esp_event_handler_instance_register(ESP_EVENT_ANY_BASE, ESP_EVENT_ANY_ID,
  ...
  wifi_init_sta("AeroNet", "1tsRuffGrounds!");
...
void start_server() {
  ...
  xTaskCreatePinnedToCore(tcp_server_task, "tcp_server", 1 * 4096,
                          (void *)AF_INET, 21, NULL, 1);
}
